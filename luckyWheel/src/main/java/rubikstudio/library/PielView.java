...
public class PielView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int i = 0; i < mLuckyItemList.size(); i++) {
            ...
            // Tính toán khoảng cách dựa trên kích thước pixel của 1dp
            float spacing = 1 * getResources().getDisplayMetrics().density;

            // Vẽ cung với khoảng cách giữa mỗi mục
            canvas.drawArc(mRange, tmpAngle, sweepAngle - spacing, true, mArcPaint);

            drawText(canvas, tmpAngle, sweepAngle - spacing, mLuckyItemList.get(i).text);
    ...
    private void drawText(Canvas canvas, float tmpAngle, float sweepAngle, String mStr) {
        ...
        float textHeight = mTextPaint.getTextSize(); // Lấy chiều cao của văn bản

        // Tính toán giá trị căn chỉnh theo chiều ngược kim đồng hồ hoặc chiều kim đồng hồ
        int hOffset = (int) (mRadius * Math.PI / mLuckyItemList.size() / 2);
        int vOffset = (int) (textHeight / 2); // Đặt vOffset để giữa văn bản theo chiều dọc

        // Tính toán góc để quay văn bản dọc theo đường cung
        float angle = (tmpAngle + sweepAngle / 2 + 360) % 360;

        // Xoay canvas để văn bản theo chiều của đường cung
        canvas.rotate(angle, getWidth() / 2, getHeight() / 2);

        // Vẽ văn bản trên đường cung
        canvas.drawText(mStr, getWidth() / 2 + hOffset, getHeight() / 2 - vOffset, mTextPaint);

        // Đặt lại góc xoay để trả về trạng thái ban đầu của canvas
        canvas.rotate(-angle, getWidth() / 2, getHeight() / 2);
    }
